...
function process(srcs) {
	...
	$.getImgs(srcs, function() {
		...
		var paletteQuantizer;
		imgs.forEach(function (img, index) {
			var id = baseName(img.src)[0];

			ti.mark("sample '" + id + "'", function(){
				if(opts.paletteQuantizer === "neuquant") {
					paletteQuantizer = new ColorQuantization.PaletteQuantizers.NeuQuant(opts.colors);
				} else {
					paletteQuantizer = new ColorQuantization.PaletteQuantizers.RgbQuant(opts.colors);
				}
				paletteQuantizer.sample(pointBuffers[index]);
			});
		});
...
*/
				var imageQuantizer;
				if(opts.dithKern === -1) {
					imageQuantizer = new ColorQuantization.ImageQuantizers.Nearest();
				} else {
					imageQuantizer = new ColorQuantization.ImageQuantizers.ErrorDiffusionDithering(opts.dithKern);
				}

				img8 = imageQuantizer.quantize(pointBuffers[index], pal8).exportUint8Array();
			});
